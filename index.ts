let students = [
  {
    ...
    avatar:
      "https://robohash.org/utdoloribusquibusdam.png?size=300x300&set=set1",
    gender: "Genderfluid",
  ...
  },
  {
    ...
    avatar:
      "https://robohash.org/voluptatemquasiconsectetur.png?size=300x300&set=set1",
    gender: "Genderfluid",
  ...
  },
  {
    ...
    avatar:
      "https://robohash.org/quinecessitatibusdoloremque.png?size=300x300&set=set1",
    gender: "Genderqueer",
  ...
  },
  {
    ...
    avatar:
      "https://robohash.org/asperioresutrepellat.png?size=300x300&set=set1",
    gender: "Bigender",
...
// ALINEA 2
function getFlag(studentObject, flagsArray): string | undefined {
  let studentFlag = flagsArray.find(
    (flag) => flag.country === studentObject.address.country
  );

  return studentFlag?.flag;
}

// ALINEA 3
function getAge(studentObject): number {
  let currentYear = new Date().getFullYear();
  let birthday = new Date(studentObject.date_of_birth).getFullYear();
...
let newStudents: Student[] = students.map((student) => {
  return {
...
newStudents.forEach((student) => {
  ...
  <img src="${student.flag}" alt="${student.country} flag" class="student-flag">
  <div>
...
// ALINEA 5
function getAverageAge(array: Student[]): number {
  let totalAge = array.reduce((acc, curr) => acc + curr.age, 0);
  let totalStudents = array.length;
  let averageAge = totalAge / totalStudents;
  return averageAge;
}

function studentsPerCountry(array: Student[]): string {
  let countryCounter = {};
  let studentsPerCountryString: string = "";
  ...
  for (const key in countryCounter) {
    studentsPerCountryString =
      studentsPerCountryString + `${key}: ${countryCounter[key]} | `;
  }

  return studentsPerCountryString.slice(0, -2);
}

let studentsDataContainer = document.querySelector(
  ".students-data-container"
) as HTMLElement;
studentsDataContainer.innerHTML = `<div><b>Average age:</b> ${getAverageAge(
  newStudents
)}</div>
<div><b>Students per country:</b> ${studentsPerCountry(newStudents)}</div>`;
